...
int[] selectedChannels = { 1, 3, 16 };

OscP5 oscP5;
float[] analogInputs = new float[selectedChannels.length];
...
void draw() {
  ...
  for (int i = 0; i < selectedChannels.length; i++) {
    ...
    float yPos = map(analogInputs[selectedChannels[i]], 0, 1, i * graphHeight + graphHeight, i * graphHeight);
    if (analogInputs[selectedChannels[i]] == 0) strokeWeight(10);
    else {
      strokeWeight(1);
    }
...
void oscEvent(OscMessage theOscMessage) {
  ...
  //if (addrPattern.equals("/inputs/digital")) {
  //  for (int i = 0; i < analogInputs.length; i ++) {
  //    analogInputs[i] = theOscMessage.get(i).intValue();
  //  }
  //}
